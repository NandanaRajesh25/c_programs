...
// Function to write NAMTAB to a file
void WRITE_NAMTAB() {
    ...
    fprintf(namtab_file, "Index\tName\tStart\tEnd\n");
    for (int i = 0; i < namcount; i++) {
        fprintf(namtab_file, "%d\t%s\t%d\t%d\n", i, mynamtab[i].name, mynamtab[i].start, mynamtab[i].end);
    }
    fclose(namtab_file);
    printf("NAMTAB written to namtab.txt\n");
}

// Function to write DEFTAB to a file
void WRITE_DEFTAB() {
    ...
    fprintf(deftab_file, "Index\tMacro Line\n");
    for (int i = 0; i < defcount; i++) {
        fprintf(deftab_file, "%d\t%s\n", i, mydeftab[i].macroline);
    }
    fclose(deftab_file);
    printf("DEFTAB written to deftab.txt\n");
}

// Function to write ARGTAB to a file
void WRITE_ARGTAB() {
    ...
    fprintf(argtab_file, "Index\tArgument\n");
    for (int i = 0; i < MAX_ARGS && myargtab[i].arg[0] != '\0'; i++) {
        fprintf(argtab_file, "%d\t%s\n", i, myargtab[i].arg);
    }
    fclose(argtab_file);
